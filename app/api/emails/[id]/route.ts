...
function extractId(request: NextRequest): number {
  ...
  const idMatch = pathname.match(/\/api\/emails\/(\d+)/);
...
async function GETHandler(request: NextRequest) {
  ...
  if (!email) throw new AppError('Email no encontrado', 404, 'NOT_FOUND');
...
async function PUTHandler(request: NextRequest) {
  ...
  const id = extractId(request);
  const body = await validateRequestBody(request, actualizarEmailSchema);
...
async function DELETEHandler(request: NextRequest) {
  ...
  return NextResponse.json({ message: 'Email eliminado' });
}
